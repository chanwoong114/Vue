<template>
  <div class="text-start">
    <h1>오늘 할일</h1>
    <span @click="createList" style="font-size: 25px;">+</span> &nbsp; <input type="text" v-model="todoContent">
    <hr>
    <div v-for="(todo, index) in selectTodayTodo" :key="index"
    class="border border-black rounded my-2 d-flex justify-content-between">
    <div>
...
export default {
  ...
  computed: {
    selectTodayTodo() {
      console.log(this.$store.getters.todayTodo)
  ...
  methods: {
    createList() {
      this.$store.dispatch('createList', [this.todoContent, false], )
      this.todoContent = null
    },
    ...mapActions(['selectImportant']),
  }
}
</script>
